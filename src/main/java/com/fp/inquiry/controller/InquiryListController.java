...
public class InquiryListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 헤더부분에 있는 헤드셋아이콘 클릭시 /list.in?page=1		=> 공지사항 1번페이지 요청
		// 페이징바에 있는 페이지번호 클릭시 /list.in?page=클릭한번호	=> 클릭한페이지 요청
		...
		// listCount : 총 문의사항 갯수 (db로부터 조회)
		listCount = new InquiryService().selectListCount(memNo);
		// currentPage : 사용자가 요청한(클릭) 페이지 수 (현재 띄워줄 페이지 번호)
		currentPage = Integer.parseInt(request.getParameter("page"));
		// pageLimit : 페이징바의 페이지 최대 갯수 (페이징바의 목록수  단위)
		pageLimit = 5;
		// boardLimit : 문의사항의 최대 갯수(문의사항 노출 단위)
		boardLimit = 10;
		...
		List<Inquiry> list = new InquiryService().selectList(pi, memNo);

		request.setAttribute("pi", pi);
		request.setAttribute("list", list);
		request.getRequestDispatcher("/views/serviceCenter/inquiryList.jsp").forward(request, response);
	}
