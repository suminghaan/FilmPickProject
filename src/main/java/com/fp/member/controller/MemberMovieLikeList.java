...
/**
 * Servlet implementation class MemberMovieLikeList
 */
@WebServlet("/movieLikeList.me")
public class MemberMovieLikeList extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public MemberMovieLikeList() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 페이징처리
				...
				List<Movie> list = new MemberService().selectMovieList(memNo,pi);
				
				request.setAttribute("memNo", memNo);
				request.setAttribute("pi", pi);
				request.setAttribute("likelist", list);		
				request.getRequestDispatcher("/views/mypage/movieLikeList.jsp").forward(request, response);
	}
