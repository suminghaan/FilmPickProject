...
/**
 * Servlet implementation class MemberMovieStarRatingList
 */
@WebServlet("/movieStarRatingList.me")
public class MemberMovieStarRatingList extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public MemberMovieStarRatingList() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<Movie> list = new MemberService().selectMovieStarRatingList(memNo,pi);
						
		request.setAttribute("memNo", memNo);
		request.setAttribute("pi", pi);
		request.setAttribute("starratinglist", list);				
		request.getRequestDispatcher("/views/mypage/movieStarRatingList.jsp").forward(request, response);
	}
